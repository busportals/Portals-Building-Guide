...
The Pistol is a versatile weapon that fires projectiles. It uses hitscan mechanics for instant hit detection.

### Damage Settings

#### Damage Falloff

The pistol's damage varies based on distance from the target:
...
* Reload One By One – When enabled, loads rounds individually (can interrupt to fire). When disabled, reloads the full clip at once
